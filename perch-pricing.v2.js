...
(() => {
  ...
  function calculateHardwarePrice(element, priceType) {
    return getElements(element, "[pp='hardware-item']")
      ...
      .reduce((total, item) => {
        let priceText = getElement(item, priceType === "cash" ? "[pp='hardware-price-cash']" : "[pp='hardware-price-haas']")?.innerText.trim() || "";
        let price = Number(removePrefix(priceText, "$").trim().replace(/,/g, ""));
        return total + price;
      }, 0);
  }
  ...
  const quantityButtons = getElements(section2, select("quantity-counter-button"));
  const quantityDisplays = getElements(section2, select("quantity-display"));
  const selectedPlanDisplays = getElements(section2, select("selected-plan-display"));
  const planCardWrappers = getElements(section2, select("plan-card-wrapper"));
  const planRadioButtons = planCardWrappers.flatMap(wrapper => getElements(wrapper, "input[type='radio']"));
  ...
  function updatePricing() {
    ...
    let selectedPlanCardWrapper = document.querySelector(`input[type='radio'][value='${selectedPlan}']`).closest(select("plan-card-wrapper"));
    if (!selectedPlanCardWrapper) throw new Error("no selected plan card wrapper");
    ...
    let softwareCost = getPlanDetails(selectedPlanCardWrapper).priceByQuantity.get(quantity);
